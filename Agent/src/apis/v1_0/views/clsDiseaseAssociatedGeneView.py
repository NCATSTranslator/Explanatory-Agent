...
from apis.v1_0.views.QueryHandlers import geneticsProvider
from apis.common.utils.modTextUtils import isNullOrWhiteSpace
from collections import OrderedDict


class DiseaseAssociatedGeneView:
    explanations = {
    ...
    def __init__(self, query: clsQuery):
        """
        query - Query object to fill with data. Assumed the query graph has already been filled in.
        """
        self.query = query
        assert self.query.query_graph is not None
    ...
    def supports_query(self, trapi_query):
        ...
        geneId = None
        diseaseId = None
        validCuries = DiseaseAssociatedGeneView.get_valid_curies()
    ...
    def retrieve(self):
        ...
        """
        query_message = self.query.generateQueryOnlyMessage()

        gp = geneticsProvider.GeneticsProvider()
        self.query_results = gp.query(query_message)

    def populate_knowledge_graph(self):
        self.query.applyKnowledgeGraphFromKnowledgeProviderResponseBody(self.query_results)
    ...
    def create_results(self):
        ...
        self.query.results = []
        for knowledgeEdgeId, knowledgeEdge in self.query.knowledge_graph['edges'].items():
            explanationCode = DiseaseAssociatedGeneView.resolveTargetIdExplanationCount(targetIdExplanationCounts[knowledgeEdge['object']])

            result = OrderedDict()
            result['edge_bindings'] = {"e00": [{
                "id": knowledgeEdgeId,
                "Explanatory_Agent_Ranking": explanationCode,
                "Explanatory_Agent_Explanation": DiseaseAssociatedGeneView.explanations[explanationCode]
    ...
    def generateTargetIdExplanationCounts(self):
        ...
        for edgeId, knowledgeEdge in self.query.knowledge_graph['edges'].items():
            targetId = knowledgeEdge['object']
            if targetId not in targetIdExplanationCounts:
                targetIdExplanationCounts[targetId] = {code: 0 for code in DiseaseAssociatedGeneView.explanations.keys()}  # initialize keys with 0 as value
    ...
    def validate(self):
        ...
        self.query.userResponseBodyIsValid(response_message)
