...
class test_clsDiseaseToChemicalQuery(unittest.TestCase):
    ...
    def test_user_request_body_is_supported(self):
        query = clsDiseaseToChemicalQuery(userRequestBody=self.loadJsonFromFile("test_user_request_body_nominal.json"))
        result = query.userRequestBodyIsSupported()
        self.assertEqual(True, result)

    def test_user_request_body_not_supported(self):
        query = clsDiseaseToChemicalQuery(userRequestBody=self.loadJsonFromFile("test_user_request_body_not_supported.json", directoryAlias="common"))
        result = query.userRequestBodyIsSupported()
    ...
    def test_query_view(self):
        ...
        with requests_mock.Mocker() as mocker:
            mocker.register_uri('POST', clsMolecularDataKnowledgeProvider.url, json=knowledgeProviderResponseBody)
    ...
    def test_query_view_knowledge_provider_http_error(self):
        ...
        with requests_mock.Mocker() as mocker:
            mocker.register_uri('POST', clsMolecularDataKnowledgeProvider.url, exc=requests.exceptions.HTTPError("Bad request"))
            response = self.client.post(f'/{version}/query', json=userRequestBody, follow_redirects=True)
        self.assertEqual(500, response.status_code)
        self.assertEqual(userResponseBody, response.json)
