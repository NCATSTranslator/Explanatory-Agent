...
class test_clsDiseaseAssociatedGeneQuery(unittest.TestCase):
    ...
    def test_user_request_body_is_supported(self):
        query = clsDiseaseAssociatedGeneQuery(userRequestBody=self.loadJsonFromFile("test_user_request_body_nominal.json"))
        result = query.userRequestBodyIsSupported()
        self.assertEqual(True, result)

    def test_user_request_body_not_supported(self):
        query = clsDiseaseAssociatedGeneQuery(userRequestBody=self.loadJsonFromFile("test_user_request_body_not_supported.json", directoryAlias="common"))
        result = query.userRequestBodyIsSupported()
    ...
    def test_query_view(self):
        ...
        with requests_mock.Mocker() as mocker:
            mocker.register_uri('POST', clsGeneticsKnowledgeProvider.url, json=knowledgeProviderResponseBody)
