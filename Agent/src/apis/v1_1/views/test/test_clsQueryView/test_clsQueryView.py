...
class test_clsQueryView(unittest.TestCase):
    ...
    def test_user_request_body_nominal_case_id_not_found_not_supported(self):
        ...
        # findMostSimilarCase
        sqlFindMostSimilarCase = clsCaseSolutionManager.sqlFindMostSimilarCase
        paramsFindMostSimilarCase = {
        ...
        resultsFindMostSimilarCase = None

        when(db.session).\
            execute(statement=sqlFindMostSimilarCase, params=paramsFindMostSimilarCase).\
            thenReturn(MagicMock())  # mock cursor
        when(db.session.execute(statement=sqlFindMostSimilarCase, params=paramsFindMostSimilarCase)).\
            fetchone().\
            thenReturn(resultsFindMostSimilarCase)  # mock results
    ...
    def test_user_request_body_nominal_knowledge_provider_crash(self):
        ...
        # findCaseSolution
        sqlFindCaseSolution = clsCaseSolutionManager.sqlFindCaseSolution
        paramsFindCaseSolution = {
            "caseIdVar": "Q000019"
        }
        resultsFindCaseSolution = {
            "Id": 1271,
        ...
        when(db.session).\
            execute(statement=sqlFindCaseSolution, params=paramsFindCaseSolution).\
            thenReturn(MagicMock())  # mock cursor
        when(db.session.execute(statement=sqlFindCaseSolution, params=paramsFindCaseSolution)).\
            fetchone().\
            thenReturn(resultsFindCaseSolution)  # mock results
