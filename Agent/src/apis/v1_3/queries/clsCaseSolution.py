...
class clsCaseSolution(clsElement):
    ...
    def generateQueryGraphForOnePath(self):
        ...
        query_graph['edges'][self.predicate_query_graph_edge_id]['predicates'] = [path.predicate]
        query_graph['nodes'][self.subject_query_graph_node_id]['categories'] = [path.subject]
        query_graph['nodes'][self.object_query_graph_node_id]['categories'] = [path.object]
        ...
        if self.edgeKnowledgeType is not None:
            query_graph['edges'][self.predicate_query_graph_edge_id]['knowledge_type'] = deepcopy(self.edgeKnowledgeType)
        if self.edgeAttributeConstraints is not None:
            query_graph['edges'][self.predicate_query_graph_edge_id]['attribute_constraints'] = deepcopy(self.edgeAttributeConstraints)
        if self.edgeQualifierConstraints is not None:
            query_graph['edges'][self.predicate_query_graph_edge_id]['qualifier_constraints'] = deepcopy(self.edgeQualifierConstraints)
