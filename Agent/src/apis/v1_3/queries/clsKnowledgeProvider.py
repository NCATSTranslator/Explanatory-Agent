    ...
    def validateResponseBody(self):
        ...
        try:
            ...
            self.logs.append(clsLogEvent(
                ...
                message=f"Knowledge Provider {self.url} did not return a valid TRAPI v{trapi_version} response for message: {self.requestBody}"
            ))
            error_message = str(e).split('\n')[0]
            raise requests.exceptions.InvalidSchema(f"Knowledge Provider {self.url} did not return a valid TRAPI v{trapi_version} response: {error_message} for message: {self.requestBody}")  # force raise a request.exception to know its the KP's fault
